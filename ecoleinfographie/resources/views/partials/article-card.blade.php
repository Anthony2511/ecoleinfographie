	...
	<footer class="blog-card__footer">
		...
		<a href="{{ Url('/').'/'.trans('url.teachers').'/'.$article->teacher->slug }}" class="blog-card__author" rel=author>
			<img src="{{ $article->teacher->getImageProfile('_30x30.jpg') }}"
					 ...
					 width="30"
					 height="30"
					 alt="Photo de {{ $article->teacher->fullname }}, {{ strtolower($article->teacher->role) }} à la Haute École de la Province de Liège" class="blog-card__author__img">
		...
		@if($article->author)
			<span class="blog-card__author blog-card__author--noprofil" rel=author>
				<img src="{{ $article->author->getImageAuthor('_30x30.jpg') }}"
						 ...
						 width="30"
						 height="30"
						 alt="Photo de {{ $article->author->fullname }}, un auteur sur ce blog" class="blog-card__author__img">
